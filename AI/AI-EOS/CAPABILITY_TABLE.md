...
| `superpowers:finishing-a-development-branch` | Yes | Yes | Apply the authorized non-integration outcome | Existing scope selected option 3: preserve `milestone-2-working-mvp` and its worktree; no merge, push, deployment, tag, cleanup, or next milestone. |
